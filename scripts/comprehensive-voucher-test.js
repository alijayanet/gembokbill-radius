const sqlite3 = require('sqlite3').verbose();
const path = require('path');

// Path to the database
const dbPath = path.join(__dirname, '..', 'data', 'billing.db');

// Open the database
const db = new sqlite3.Database(dbPath, (err) => {
  if (err) {
    console.error('Error opening database:', err.message);
    process.exit(1);
  }
  
  console.log('Connected to the database.');
  
  // Test the exact query that's failing in the application
  const testQuery = `
    INSERT OR REPLACE INTO voucher_online_settings
    (package_id, name, profile, digits, price, enabled, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, datetime('now'))
  `;
  
  const testValues = ['test-complete', 'Complete Test Package', 'test-profile', 6, 15000, 1];
  
  db.run(testQuery, testValues, function(err) {
    if (err) {
      console.error('❌ Error with complete voucher settings query:', err.message);
      db.close();
      process.exit(1);
    }
    
    console.log('✅ Successfully executed complete voucher settings query');
    console.log(`   Rows affected: ${this.changes}`);
    
    // Verify the data was inserted correctly
    db.get('SELECT * FROM voucher_online_settings WHERE package_id = ?', ['test-complete'], (err, row) => {
      if (err) {
        console.error('Error retrieving test data:', err.message);
        db.close();
        process.exit(1);
      }
      
      if (row) {
        console.log('✅ Test data verification:');
        console.log(`   Package ID: ${row.package_id}`);
        console.log(`   Name: ${row.name}`);
        console.log(`   Profile: ${row.profile}`);
        console.log(`   Digits: ${row.digits}`);
        console.log(`   Price: ${row.price}`);
        console.log(`   Enabled: ${row.enabled}`);
        console.log(`   Updated at: ${row.updated_at}`);
        
        // Clean up test data
        db.run('DELETE FROM voucher_online_settings WHERE package_id = ?', ['test-complete'], (err) => {
          if (err) {
            console.error('Warning: Could not clean up test data:', err.message);
          } else {
            console.log('✅ Test data cleaned up successfully');
          }
          
          db.close();
          console.log('\n🎉 All tests passed! The voucher settings should now work correctly.');
          process.exit(0);
        });
      } else {
        console.error('❌ Test data not found');
        db.close();
        process.exit(1);
      }
    });
  });
});