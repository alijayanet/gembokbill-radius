...
router.get('/system/check-update', async (req, res) => {
    try {
        ...
        const repoUrl = getSetting('git_repository_url', 'https://github.com/alijayanet/gembokbill-radius');
        const opts = { cwd: repoPath, windowsHide: true, shell: process.platform === 'win32' ? undefined : '/bin/bash' };

        // Set remote URL if configured
        exec(`git remote set-url origin ${repoUrl}`, opts, (errSetUrl) => {
            if (errSetUrl) {
                // If setting remote fails, try to add it
                exec(`git remote add origin ${repoUrl}`, opts, (errAddUrl) => {
                    if (errAddUrl) {
                        // Ignore error, remote might already exist
                    }
                    // Continue with fetch regardless of set-url result
                ...
                // Set-url successful, proceed with fetch
                exec('git fetch --all --prune', opts, (errFetch, outFetch, errFetchStderr) => {
                    if (errFetch) {
                        return res.status(500).json({ success: false, message: 'Check update failed', error: errFetchStderr || errFetch.message, log: outFetch, repoPath });
                    }
        ...
        exec(`git remote set-url origin ${repoUrl}`, opts, (errSetUrl) => {
            if (errSetUrl) {
                ...
                exec(`git remote add origin ${repoUrl}`, opts, (errAddUrl) => {
                    ...
                    // Continue with update regardless of set-url result
                    runUpdate();
                });
            } else {
                // Set-url successful, proceed with update
                runUpdate();
            }
            ...
            function runUpdate() {
                ...
                exec(updateCmd, opts, (errUpdate, outUpdate, errUpdateStderr) => {
                    if (errUpdate) {
                        return res.status(500).json({ success: false, message: 'Update failed', error: errUpdateStderr || errUpdate.message, log: outUpdate, repoPath });
                    }
                    const pm2Target = getSetting('pm2_restart_target', appName);
                    if (!pm2Target) {
                        return res.json({ success: true, message: 'Update completed (PM2 restart skipped - no target)', log: outUpdate, repoPath });
                    }
                    const pm2Cmd = `pm2 restart ${pm2Target} || pm2 reload ${pm2Target}`;
                    exec(pm2Cmd, opts, (pm2Err, pm2Out, pm2ErrStderr) => {
                        if (pm2Err) {
                            return res.status(200).json({ success: true, message: 'Update completed, PM2 restart failed', log: outUpdate + '\n' + pm2Out, pm2Error: pm2ErrStderr || pm2Err.message, repoPath });
                        }
                        exec('pm2 save', opts, () => {
                            return res.json({ success: true, message: 'Update completed & PM2 restarted', log: outUpdate + '\n' + pm2Out, repoPath });
                        });
                    });
                });
            }
