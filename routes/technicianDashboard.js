...
/**
 * Get Payment Detail - Mendapatkan detail payment untuk view/edit
 */
router.get('/payments/:id', technicianAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const collectorId = req.technician.id;
        ...
        const payment = await new Promise((resolve, reject) => {
            const sql = `
                ...
                WHERE cp.id = ? AND cp.collector_id = ?
            `;
            
            db.get(sql, [id, collectorId], (err, row) => {
                if (err) reject(err);
                else resolve(row);
            });
        });
        ...
        logger.error('Error fetching payment detail:', error);
        res.status(500).json({ 
            success: false, 
            message: 'Gagal memuat detail payment: ' + error.message 
        });
    }
});

/**
 * Update Payment - Update payment yang sudah dicatat (hanya pending)
 */
router.put('/payments/:id', technicianAuth, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (!amount || !payment_method) {
            return res.status(400).json({ 
                success: false, 
                message: 'Amount dan metode payment wajib diisi' 
            });
        }
        ...
        const existingPayment = await new Promise((resolve, reject) => {
            const sql = `SELECT * FROM collector_payments WHERE id = ? AND collector_id = ?`;
            db.get(sql, [id, collectorId], (err, row) => {
                if (err) reject(err);
                else resolve(row);
            });
        ...
        // Hanya boleh edit payment yang masih pending
        if (existingPayment.status !== 'pending') {
            return res.status(400).json({ 
                success: false, 
                message: 'Hanya payment dengan status pending yang bisa diedit' 
            });
        }
        ...
        await new Promise((resolve, reject) => {
            const sql = `
                ...
                SET amount = ?, payment_method = ?, reference_number = ?, notes = ?
        ...
        res.status(500).json({ 
            success: false, 
            message: 'Gagal mengupdate payment: ' + error.message 
        });
    }
});
