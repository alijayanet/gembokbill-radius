...
class ServiceSuspensionManager {
    ...
    async suspendCustomerService(customer, reason = 'Telat bayar') {
        try {
            ...
            // 5. Sync to RADIUS if enabled
            if (customer.radius_enabled && (customer.radius_username || customer.pppoe_username)) {
                try {
                    const rUsername = customer.radius_username || customer.pppoe_username;
                    const isolirGroup = getSetting('isolir_radius_group', 'isolir');
                    
                    // Update user RADIUS ke group isolir
                    await radius.updateRadiusUser(rUsername, null, {
                        'Framed-Pool': 'isolir-pool',
                        'Mikrotik-Rate-Limit': getSetting('suspension_bandwidth_limit', '1k/1k')
                    });
                    
                    // Tambah user ke group isolir di RADIUS
                    await radius.addUserToGroup(rUsername, isolirGroup, 1);
                    
                    logger.info(`RADIUS: Customer ${customer.username} (${rUsername}) added to isolir group '${isolirGroup}'`);
                    results.radius = true;
                } catch (radiusError) {
    ...
    async restoreCustomerService(customer, reason = 'Manual restore') {
        try {
            ...
            // 5. Restore RADIUS if enabled
            if (customer.radius_enabled && (customer.radius_username || customer.pppoe_username)) {
                try {
                    const rUsername = customer.radius_username || customer.pppoe_username;
                    const isolirGroup = getSetting('isolir_radius_group', 'isolir');
                    const defaultGroup = customer.radius_group || 'default';
                    
                    // Hapus user dari group isolir di RADIUS
                    const pool = radius.getRadiusPool();
                    await pool.query('DELETE FROM radusergroup WHERE username = ? AND groupname = ?', [rUsername, isolirGroup]);
                    
                    // Update user RADIUS dengan attributes normal
                    let rAttributes = {};
                    if (customer.radius_attributes) {
                        try {
                            rAttributes = JSON.parse(customer.radius_attributes);
                        } catch (e) {
                            logger.error('Error parsing radius_attributes:', e);
                        }
                    }
                    
                    await radius.updateRadiusUser(rUsername, null, rAttributes);
                    
                    logger.info(`RADIUS: Customer ${customer.username} (${rUsername}) removed from isolir group '${isolirGroup}' and restored to '${defaultGroup}'`);
                    results.radius = true;
                } catch (radiusError) {
                    logger.error(`RADIUS restoration failed for ${customer.username}:`, radiusError.message);
                }
            }
