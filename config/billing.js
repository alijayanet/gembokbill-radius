...
class BillingManager {
    ...
    initDatabase() {
        // Database connection handled by config/database.js on demand
        // but we can call it here to ensure it's ready
        this.db.init().then(() => {
            console.log('✅ Billing database wrapper initialized');
            this.createTables();
        }).catch(err => {
            console.error('❌ Error initializing database wrapper:', err);
        });
    }

    async updateCustomerById(id, customerData) {
        return new Promise(async (resolve, reject) => {
            const { name, username, pppoe_username, email, address, latitude, longitude, package_id, odp_id, pppoe_profile, status, auto_suspension, billing_day, cable_type, cable_length, port_number, cable_status, cable_notes, radius_enabled, radius_username, radius_password, radius_group, radius_attributes } = customerData;
            try {
                ...
                const sql = `UPDATE customers SET name = ?, username = ?, pppoe_username = ?, email = ?, address = ?, latitude = ?, longitude = ?, package_id = ?, odp_id = ?, pppoe_profile = ?, status = ?, auto_suspension = ?, billing_day = ?, cable_type = ?, cable_length = ?, port_number = ?, cable_status = ?, cable_notes = ?, radius_enabled = ?, radius_username = ?, radius_password = ?, radius_group = ?, radius_attributes = ? WHERE id = ?`;
                this.db.run(sql, [
                    ...
                    radius_enabled !== undefined ? (radius_enabled ? 1 : 0) : (oldCustomer.radius_enabled ?? 0),
                    radius_username !== undefined ? radius_username : oldCustomer.radius_username,
                    radius_password !== undefined ? radius_password : oldCustomer.radius_password,
                    radius_group !== undefined ? radius_group : (oldCustomer.radius_group ?? 'default'),
                    radius_attributes !== undefined ? radius_attributes : oldCustomer.radius_attributes,
                    id
                ], async function (err) {
                    if (err) {
                        reject(err);
                        ...
                        if (odp_id !== undefined || cable_type !== undefined) {
                            ...
                            try {
                                ...
                                if (existingRoute) {
                                    ...
                                    db.run('UPDATE cable_routes SET odp_id = ?, cable_type = ?, cable_length = ?, port_number = ?, status = ?, notes = ? WHERE customer_id = ?',
                                        [odp_id || existingRoute.odp_id, cable_type || existingRoute.cable_type, cable_length || existingRoute.cable_length, port_number || existingRoute.port_number, cable_status || existingRoute.status, cable_notes || existingRoute.notes, customerId],
                                        (err) => {
                                            if (err) console.error('❌ Error updating cable route:', err);
                                            else console.log('✅ Cable route updated successfully');
                                        });
                                } else if (odp_id) {
                                    // Create new cable route
                                    db.run('INSERT INTO cable_routes (customer_id, odp_id, cable_type, cable_length, port_number, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?)',
                                        [customerId, odp_id, cable_type || 'Fiber Optic', cable_length || 0, port_number || 1, cable_status || 'connected', cable_notes || `Created for customer ${name}`],
                                        (err) => {
                                            if (err) console.error('❌ Error creating cable route:', err);
                                            else console.log('✅ Cable route created successfully');
                                        });
                                }
                            } catch (cableError) {
                        ...
                        if (radius_enabled && (radius_username || pppoe_username) && radius_password) {
                            try {
                                const radius = require('./radius');
                                const rUsername = radius_username || pppoe_username;
                                let rAttributes = {};
                                if (radius_attributes) {
                                    try {
                                        rAttributes = JSON.parse(radius_attributes);
                                    } catch (e) {
                                        console.error('Error parsing radius_attributes:', e);
                                    }
                                }
                                await radius.addRadiusUser(rUsername, radius_password, rAttributes);
                                console.log(`✅ Customer ${oldCustomer.username} synced to RADIUS as ${rUsername}`);
                            } catch (radiusError) {
                                console.error('❌ Error syncing to RADIUS:', radiusError.message);
                            }
                        }
    ...
    createTables() {
        ...
        // payment_status column sudah ada di migration 00_init_core_billing_tables.sql
        // Tidak perlu ditambahkan lagi untuk menghindari duplicate column error
        ...
        // Commented out - column already exists in migration
        ...
        // Commented out because pppoe_profile already exists in migration
        ...
        // Commented out because cable fields already exist in migration
        ...
        // Commented out - columns already exist in migration
    ...
    addColumnsAndCreateIndexes() {
        // Commented out all ALTER TABLE statements because columns already exist in migrations
        // These were causing duplicate column errors
        ...
        return new Promise(async (resolve, reject) => {
            ...
            db.run(sql, [finalUsername, name, phone, autoPPPoEUsername, email, address, package_id, customerData.odp_id || null, pppoe_profile, status || 'active', auto_suspension !== undefined ? auto_suspension : 1, normBillingDay, static_ip || null, assigned_ip || null, mac_address || null, finalLatitude, finalLongitude, cable_type || null, cable_length || null, port_number || null, cable_status || 'connected', cable_notes || null, radius_enabled ? 1 : 0, radius_username || null, radius_password || null, radius_group || 'default', radius_attributes || null], async function (err) {
                if (err) {
                    ...
                    // Sync to RADIUS if enabled
                    if (radius_enabled && (radius_username || autoPPPoEUsername) && radius_password) {
                        try {
                            const radius = require('./radius');
                            const rUsername = radius_username || autoPPPoEUsername;
                            let rAttributes = {};
                            if (radius_attributes) {
                                try {
                                    rAttributes = JSON.parse(radius_attributes);
                            ...
                            await radius.addRadiusUser(rUsername, radius_password, rAttributes);
                            console.log(`✅ Customer ${finalUsername} synced to RADIUS as ${rUsername}`);
                        } catch (radiusError) {
                            console.error('❌ Error syncing to RADIUS:', radiusError.message);
                        }
                    }
